...
def get_thumbnail_by_name(db, name):
    ...
    return db.query(Archive.thumbnail_image).filter(
            Archive.filename.like(f"{name}%")
            ).scalar()


def insert_file(db, filename, editorial, file, favorite, thumbnail_image):
    db.add(Archive(
        filename,
        editorial,
        file,
